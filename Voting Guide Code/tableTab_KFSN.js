$(document).ready(function() {
    ...
    // Load US President Race data
        loadCSV('Excel_Files/KFSN_Races/Presidential_Election_Race.csv', '#nav-us-president-table', updateImagesPresident);

        // Load US Senate Race data
    $('#nav-us-senate-tab').click(function() {
        loadCSV('Excel_Files/KFSN_Races/US_Senate_Race.csv', '#nav-us-senate-table', updateImagesUSSenate);
    });
     // Load US House Race data
     $('#nav-us-house-tab').click(function() {
        loadCSV('Excel_Files/KFSN_Races/US_House_Races.csv', '#nav-us-house-table', updateImagesUSHouse);
    });
...
function updatePresidentImage(row) {
    ...
    if (cellText === 'R') {
        ...
        img.attr('src', 'IMAGES/BALLAY-01.png').attr('alt', 'Libertarian');
...
function updateHouseImage(row) {
    ...
    // Set the image source based on the cell text
    if (cellText === 'T') {
        img.attr('src', 'IMAGES/MCCLINTOCK-01.png').attr('alt',);
        partyName = ' ';
    } else if (cellText === 'M') {
        img.attr('src', 'IMAGES/BARKLEY-01.png').attr('alt',);
        partyName = ' ';
    } else if (cellText === 'J') {
        img.attr('src', 'IMAGES/DUARTE-01.png').attr('alt',);
        partyName = ' ';
    } else if (cellText === 'A') {
        img.attr('src', 'IMAGES/GRAY-01.png').attr('alt',);
        partyName = ' ';
    }
    else if (cellText === 'V') {
        img.attr('src', 'IMAGES/FONG-01.png').attr('alt',);
        partyName = ' ';
    }
    else if (cellText === 'Mi') {
        img.attr('src', 'IMAGES/BOUDREAUX-01.png').attr('alt',);
        partyName = ' ';
    }
    else if (cellText === 'Ji') {
        img.attr('src', 'IMAGES/COSTA-01.png').attr('alt',);
        partyName = ' ';
    }
    else if (cellText === 'Mic') {
        img.attr('src', 'IMAGES/MAHER-01.png').attr('alt',);
        partyName = ' ';
    }
    else if (cellText === 'D') {
        img.attr('src', 'IMAGES/VALADAO-01.png').attr('alt',);
        partyName = ' ';
    }
    else if (cellText === 'R') {
        img.attr('src', 'IMAGES/SALAS-01.png').attr('alt',);
        partyName = ' ';
    }
    else {
        img.attr('src', 'IMAGES/DEFAULT-01.png').attr('alt', 'Unknown');
