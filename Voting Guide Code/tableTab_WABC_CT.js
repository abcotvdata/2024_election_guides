$(document).ready(function() {
  ...
  // Load US President Race data
      loadCSV('Excel_Files/WABC_CT_Races/Presidential_Election_race.csv', '#nav-us-president-table', updateImagesPresident);

      // Load US Senate Race data
  $('#nav-us-senate-tab').click(function() {
      loadCSV('Excel_Files/WABC_CT_Races/US_Senate_Race.csv', '#nav-us-senate-table', updateImagesUSSenate);
  });
   // Load US House Race data
   $('#nav-us-house-tab').click(function() {
      loadCSV('Excel_Files/WABC_CT_Races/US_House_Race.csv', '#nav-us-house-table', updateImagesUSHouse);
  });
     // Load NY Senate Race data
     $('#nav-ct-senate-tab').click(function() {
      loadCSV('Excel_Files/WABC_CT_Races/State_Senate_Race.csv', '#nav-ct-senate-table', updateImagesLADA);
  });

       // Load NY Assembly Race data
       $('#nav-ct-assembly-tab').click(function() {
        loadCSV('Excel_Files/WABC_CT_Races/State_Assembly_Race.csv', '#nav-ct-assembly-table', updateImagesLACC);
    });
...
// Function to update images for LA DA
function updateImagesLADA() {
  $('#nav-ct-senate tr').each(function() {
      if ($(this).index() === 0) return; // Skip header
      updatePartyImage($(this));
  });
}

// Function to update images for LA City Council
function updateImagesLACC() {
  $('#nav-ct-assembly tr').each(function() {
      if ($(this).index() === 0) return; // Skip header
      updatePartyImage($(this));
  });
}
...
function updateSenateImage(row) {
  ...
  // Set the image source based on the cell text
  if (cellText === 'C') {
      img.attr('src', 'IMAGES/MURPHY-01.png').attr('alt', 'Democrat');
      partyName = ' Democrat';
  } else if (cellText === 'M') {
      img.attr('src', 'IMAGES/COREY-01.png').attr('alt', 'Republican');
      partyName = ' Republican';
    } else if (cellText === 'R') {
      img.attr('src', 'IMAGES/HYDE-01.png').attr('alt', 'Independent');
      partyName = ' Independent';
  }else {
      img.attr('src', 'IMAGES/DEFAULT-01.png').attr('alt', 'Unknown');
  }
