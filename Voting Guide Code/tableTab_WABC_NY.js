$(document).ready(function() {
  ...
  $('#nav-us-senate-tab').click(function() {
      loadCSV('Excel_Files/WABC_NY_Races/US_Senate_Race.csv', '#nav-us-senate-table', updateImagesUSSenate);
  });
   // Load US House Race data
   $('#nav-us-house-tab').click(function() {
      loadCSV('Excel_Files/WABC_NY_Races/US_House_Race.csv', '#nav-us-house-table', updateImagesUSHouse);
  });
     // Load NY Senate Race data
     $('#nav-ny-senate-tab').click(function() {
      loadCSV('Excel_Files/WABC_NY_Races/State_Senate_Race.csv', '#nav-ny-senate-table', updateImagesLADA);
  });

       // Load NY Assembly Race data
       $('#nav-ny-assembly-tab').click(function() {
        loadCSV('Excel_Files/WABC_NY_Races/State_Assembly_Race.csv', '#nav-ny-assembly-table', updateImagesLACC);
    });
...
function updateImagesLADA() {
  $('#nav-ny-senate tr').each(function() {
      if ($(this).index() === 0) return; // Skip header
      updatePartyImage($(this));
  });
}
...
function updateImagesLACC() {
  $('#nav-ny-assembly tr').each(function() {
      if ($(this).index() === 0) return; // Skip header
      updatePartyImage($(this));
  });
}
...
function updateSenateImage(row) {
  ...
  // Set the image source based on the cell text
  if (cellText === 'K') {
      img.attr('src', 'IMAGES/GILLIBRAND-01.png').attr('alt', 'Democrat');
      partyName = ' Democrat';
  } else if (cellText === 'Y') {
      img.attr('src', 'IMAGES/MILLER-01.png').attr('alt', 'American Independent');
      partyName = ' American Independnet';
    } else if (cellText === 'D') {
      img.attr('src', 'IMAGES/SARE-01.png').attr('alt', 'Larouche Party');
      partyName = ' Larouche Party';
  }else {
      img.attr('src', 'IMAGES/DEFAULT-01.png').attr('alt', 'Unknown');
  }
